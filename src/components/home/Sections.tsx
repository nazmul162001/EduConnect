    ...
    <section className="container-responsive mt-16 sm:mt-20 md:mt-24 pb-16 sm:pb-20 md:pb-24 px-6 sm:px-8">
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="overflow-hidden">
          ...
          >
            {reviews.map((r) => (
              <div key={r.id} className="w-full flex-shrink-0 px-8">
                <div className="max-w-2xl mx-auto">
                  <motion.div
                    whileHover={{ y: -6, scale: 1.02 }}
                    className="relative rounded-2xl sm:rounded-3xl border border-white/20 dark:border-slate-700/50 bg-white/10 dark:bg-slate-800/30 p-6 sm:p-8 shadow-lg hover:shadow-2xl transition-all duration-300 backdrop-blur-sm"
                  >
                    {/* Quote Icon */}
        ...
        <div className="flex justify-center mt-8 gap-2">
          {reviews.map((_, reviewIndex) => (
            <button
              key={reviewIndex}
              onClick={() => setCurrentSlide(reviewIndex)}
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                reviewIndex === currentSlide
                  ? "bg-white scale-125"
